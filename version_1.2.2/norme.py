...
class norme:
    ...
    def check_tabulation(self):
        p = re.compile('(int|char|void|float|double|short|long|t_[A-Za-z0-9]+)(  )')
        test = re.search(p, self.line)
        if test:
    ...
    def check_jump_after_d(self):
        p = re.compile('^([\} \t]*)(static|size_t|pid_t|(t_[a-z]{2,})|unsigned|int|char|void|float|double|short|long)([ ]|[\t])+(.*)(;\n)')
        test = re.search(p, self.line)
        if test:
    ...
    def check_declaration_unique(self):
        ...
        test = re.search(p, self.line)
        if test:
            self.print_error('Plusieurs declarations sur la meme ligne');
            self.note += 1
    ...
    def print_error(self, msg):
        print "Erreur dans",self.file,"a la ligne",self.nb_line,":\n\033[0;31;40m",msg,"\033[0;m"
        if self.printline:
...
def help():
    ...
    print "Usage: norme.py [files_to_scan] [options]"
